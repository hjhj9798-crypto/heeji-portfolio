...
const Home: React.FC<HomeProps> = ({ projects, about, home, contact }) => {
  ...
  return (
    <div className="space-y-12 md:space-y-64 pb-12 md:pb-32">
      {/* 1. HERO SECTION */}
      <section 
        id="home" 
        className="relative flex flex-col items-center justify-center min-h-[72svh] md:min-h-[95vh] w-full mx-auto px-5 md:px-12 lg:px-16 overflow-hidden scroll-mt-20 md:scroll-mt-32 pt-20 pb-12 md:pt-24 md:pb-24 transition-all duration-500 bg-[#07090e]"
      >
...
<>
  ...
  />
        ...
        <div className="relative text-center space-y-2 md:space-y-2.5 reveal z-10 w-full max-w-7xl select-none">
          ...
          <h1 className="text-[clamp(2.4rem,13vw,5rem)] md:text-7xl lg:text-[6.75rem] xl:text-[8.1rem] 2xl:text-[9.9rem] font-bold tracking-tighter text-white uppercase leading-none whitespace-nowrap drop-shadow-[0_4px_24px_rgba(0,0,0,0.8)]">
            {home.name}
          </h1>
        </div>
      </section>
      ...
      {/* SHOWREEL SECTION */}
      <section id="showreel" className="w-full max-w-[1920px] mx-auto px-0 reveal py-6 md:py-24 scroll-mt-16 md:scroll-mt-24">
      ...
      {/* 2. PROJECTS SECTION */}
      <section id="projects" className="max-w-[1920px] mx-auto px-4 sm:px-6 md:px-16 pt-10 md:pt-32 scroll-mt-20 md:scroll-mt-32">
                ...
                <div className={`relative aspect-video bg-[#161b22] border border-white/5 overflow-hidden shadow-2xl transition-all duration-500 ${isAdditional ? 'flex items-center justify-center hover:border-blue-500/30' : 'img-hover-container'}`}>
                  {isAdditional ? (
                    ...
                    <>
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-end md:justify-center p-4 pb-5 md:p-8 opacity-100 md:opacity-0 md:group-hover:opacity-100 bg-gradient-to-t from-black/90 via-black/20 to-transparent md:bg-none transition-all duration-500 text-center z-10">
                        <h3 
                          className="text-white font-bold text-xl md:text-5xl tracking-tight uppercase transform md:translate-y-4 md:group-hover:translate-y-0 transition-transform duration-500 delay-75 mb-2 md:mb-4"
                          style={{ textShadow: '0 10px 30px rgba(0,0,0,1), 0 0 10px rgba(0,0,0,0.5)' }}
                        >
                          {project.title}
                        </h3>
                        <div className="w-12 md:w-16 h-[2px] bg-blue-500 transform md:scale-x-0 md:group-hover:scale-x-100 transition-transform duration-500 delay-150 mb-2 md:mb-4"></div>
                        <span 
                          className="text-[13px] md:text-[20px] text-blue-400 uppercase tracking-[0.15em] font-bold transform md:translate-y-4 md:group-hover:translate-y-0 transition-transform duration-500 delay-100"
                          style={{ textShadow: '0 5px 15px rgba(0,0,0,1)' }}
                        >
                          {project.year}
                        </span>
                        <p 
                          className="hidden md:flex text-[14px] text-gray-300 uppercase tracking-widest mt-4 max-w-[85%] transform translate-y-4 group-hover:translate-y-0 transition-all duration-500 delay-200 opacity-0 group-hover:opacity-100 flex-col gap-1.5"
      ...
      {/* 3. ABOUT SECTION */}
      <section id="about" className="max-w-[1920px] mx-auto px-5 sm:px-6 md:px-16 pt-10 md:pt-32 scroll-mt-20 md:scroll-mt-32">
        <div className="space-y-10 md:space-y-16 reveal">
          ...
          <div className="max-w-4xl">
            <p className="text-gray-300 leading-relaxed text-lg sm:text-xl md:text-4xl font-light border-l-2 border-blue-600/30 pl-4 md:pl-8">{about.intro}</p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-[45%_55%] gap-12 lg:gap-24 items-start pt-4 md:pt-12">
                ...
                <div className="space-y-12">
                  {about.experience.map((exp) => (
                    <div key={exp.id} className="space-y-3">
                      ...
                      <p className="text-base md:text-xl text-gray-400 leading-relaxed mt-4 max-w-2xl font-light">{exp.description}</p>
                    </div>
                  ))}
                </div>
              </section>
      ...
      {/* 4. CONTACT SECTION */}
      <section id="contact" className="max-w-[1920px] mx-auto px-5 md:px-16 pt-12 md:pt-32 scroll-mt-20 md:scroll-mt-[150px] reveal pb-16 md:pb-48 min-h-[50vh] md:min-h-[60vh] flex flex-col justify-center">
        <div className="text-center space-y-12 md:space-y-24 relative overflow-hidden">
          ...
          <div className="space-y-10 md:space-y-16">
            <a 
              ...
              className="text-[clamp(1.35rem,6vw,2.25rem)] md:text-5xl font-bold text-gray-300 pb-4 inline-block tracking-tight break-all hover:text-blue-500 transition-all hover:scale-105 transform"
            >
              {contact.email}
            </a>
            <div className="flex flex-wrap justify-center gap-x-7 md:gap-x-20 gap-y-6 text-[12px] sm:text-[14px] md:text-[22px] font-bold tracking-[0.1em] md:tracking-[0.15em]">
