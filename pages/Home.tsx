...
export default function Home({ projects, about, home, contact }: {projects: Project[]; about: AboutData; home: HomeData; contact: ContactData}) {
  return <>
    <section className="home-hero" id="home" aria-label="Heeji Woo portfolio"><h1 className="sr-only">{home.name} — {home.professionalTitle}</h1>{home.backgroundGifUrl && (/\.(mp4|webm)$/i.test(home.backgroundGifUrl) ? <LoopingVideo src={home.backgroundGifUrl} label="Portfolio cinematic background"/> : <img src={home.backgroundGifUrl} alt="Portfolio cinematic background"/>)}<div className="hero-shade"/></section>
