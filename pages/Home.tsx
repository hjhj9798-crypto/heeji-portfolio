...
export default function Home({ projects, about, home, contact }: {projects: Project[]; about: AboutData; home: HomeData; contact: ContactData}) {
  return <>
    ...
    <section className="project-grid" id="projects" aria-label="Selected work">{projects.filter(p => p.id !== 'additional-work').sort((a, b) => ['eclipse', '5', '4', '1', '3', '2'].indexOf(a.id) - ['eclipse', '5', '4', '1', '3', '2'].indexOf(b.id)).map(project => <Link key={project.id} className="project-card" to={`/projects/${project.id}`}><img src={project.id === '5' ? '/images/polish-20260904/Sol_Thumbnail.png' : project.thumbnail} alt={project.title} loading="lazy" decoding="async"/><span className="card-caption">{project.title}<small>{project.role}</small></span></Link>)}</section>
    <AdditionalCarousel/>
    <section className="about-section" id="about"><h2>ABOUT</h2><p className="about-intro">{about.intro}</p><div className="about-columns">
      ...
      <div className="skills-tools"><section className="skills-section"><h3 className="section-label">Skills</h3><ul>{about.skills?.map(skill => <li key={skill}>{skill}</li>)}</ul></section><section className="tools-section"><h3 className="section-label">Tools</h3><div className="tools-grid">{about.tools.map(category => <div key={category.category}><h4>{category.category}</h4><ul>{category.items.map(item => <li key={item}>{item}</li>)}</ul></div>)}</div></section></div></div>
    </div></section>
    <section className="contact-section" id="contact"><h2>CONTACT</h2><a className="email-link" href={`https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(contact.email)}`} target="_blank" rel="noopener noreferrer">{contact.email} ↗</a><div><a href={contact.artstationUrl} target="_blank" rel="noopener noreferrer">ArtStation ↗</a><a href={contact.linkedinUrl} target="_blank" rel="noopener noreferrer">LinkedIn ↗</a></div></section>
  </>;
}
